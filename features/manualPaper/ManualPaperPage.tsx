    ...
    const apply = (origin: HTMLElement, target: HTMLElement) => {
      ...
      const serialized = Array.from(styles)
        .map((prop) => {
          ...
          // html2canvas cannot parse modern color spaces like lab/oklab/oklch/color(display-p3)
          // Skip any declaration that uses these to avoid runtime errors; remaining styles are enough for fidelity.
          if (/lab\(|oklab\(|oklch\(|color\(display-p3/i.test(value)) {
            return "";
          }
          return `${prop}:${value};`;
        })
