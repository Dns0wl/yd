...
export function ManualPaperPage({ initialRecords }: ManualPaperPageProps) {
  ...
  const hydrateSafeColors = (node: HTMLElement | null) => {
    if (!node) return;
    const computeSafeColor = (value: string) => {
      const tester = document.createElement("div");
      tester.style.color = value;
      document.body.appendChild(tester);
      const rgb = getComputedStyle(tester).color;
      tester.remove();
      return rgb || value;
    };
    ...
    const apply = (el: HTMLElement) => {
      ...
      [
        ...
        if (!value) return;
        const safe = value.includes("lab(") ? computeSafeColor(value) : value;
        (el.style as any)[prop] = safe;
      });

      Array.from(el.children).forEach((child) => apply(child as HTMLElement));
    };
